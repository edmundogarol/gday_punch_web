...
import { paymentSubmit, paymentError, paymentSucceeded } from "actions/payment";
import { customerFetch, customerSubscribe } from "actions/customer";
...
function Checkout({ history }) {
  ...
  useEffect(() => {
    return () => {
      ...
      dispatch(paymentError(undefined));
    };
  }, []);
  ...
  useEffect(() => {
    if (
      ...
      dispatch(customerFetch(user.customer_id));
    }
  ...
  const handleSubmitPayment = async (ev) => {
    ...
    if (products.length) {
      dispatch(paymentSubmit(customerDetailsPayload, products));
    }
