...
import { selectDownloadManuscriptState } from "selectors/resources";
...
function ResourceDownloads() {
  ...
  const { errors, finished } = useSelector(selectDownloadManuscriptState);
  ...
  useEffect(() => {
    return () => {
      dispatch(resetDownloadManuscriptRequest());
    };
  }, []);
