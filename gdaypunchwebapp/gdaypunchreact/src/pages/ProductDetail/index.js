...
import { fetchViewingProduct, setViewingProduct } from "actions/products";
...
function ProductDetail({ history }) {
  ...
  useEffect(() => {
    if (!fetchingViewingProduct && !finishedFetchingViewingProduct) {
      if (productId && !product) {
        dispatch(setViewingProduct(productId));
        if (!productList[productId]) dispatch(fetchViewingProduct(productId));
      }
    }
  ...
  const handleAddToCart = () => {
    dispatch(updateCartItemQuantity(product.id, quantity, product.quantity));
  };
  ...
  const handleReadManga = () => {
    ...
    if (!loggedIn) {
      ...
      dispatch(
        doSuggestRegister("Info: Sign up or Log in to read this manga!")
      );
    } else if (unverified) {
      message.warn(
      ...
      history.push(`/manga/${product.manga_details.id}`);
    }
  };
