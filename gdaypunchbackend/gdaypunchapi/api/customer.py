...
from ..models import Customer, User, Purchase, Product, StripeCustomer
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        if not pk or pk == "null":
            ...
            try:
                ...
                return Response(
                    {"error": "User details cannot be found"},
        ...
        if dig_subscribed is not None and dig_subscribed == False:
            ...
            dig_sub_product = Product.objects.get(product_type=DIG_SUBSCRIPTION)

            Thread(
                target=send_access_updates_email_summary,
                args=(
                    customer,
                    [],
                    [
                    ...
                    "Digital subscription has been cancelled.",
                    False,
                ),
            ).start()
...
def send_access_updates_email_summary(
    customer, granting_items, removing_items, notes, signup_prompt
):
    ...
    def post(self, request, format=None):
        ...
        for product in updated_products:
            ...
            if product.get("removing", None):
                ...
                purchases = Purchase.objects.filter(customer=customer_id).filter(
                    product=product["id"]
                )
                ...
                Purchase.objects.create(
                    customer=assigning_customer, product=assigning_product
                )
        ...
        if email_summary:
            ...
            Thread(
                ...
                args=(
                    ...
                    removing_items,
                    email_notes,
                    customer.user.last_login is None,
                ),
            ).start()
